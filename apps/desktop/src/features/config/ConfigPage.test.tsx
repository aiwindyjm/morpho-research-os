...
describe("ConfigPage (prototype numbered sections)", () => {
  ...
  it("exposes the segmented depth control as a named radiogroup", async () => {
    renderPage();
    await screen.findByLabelText(/研究主题/);
    const group = screen.getByRole("radiogroup", { name: "研究深度" });
    expect(group).toBeInTheDocument();
    expect(screen.getByLabelText(/研究深度/)).toBeInTheDocument();
  });

  it("depth radiogroup keeps roving-tabindex, aria-checked, and arrow-key selection", async () => {
    const user = userEvent.setup();
    renderPage();
    await screen.findByLabelText(/研究主题/);

    const radios = within(
      screen.getByRole("radiogroup", { name: "研究深度" }),
    ).getAllByRole("radio");
    expect(radios).toHaveLength(5);

    // Fixture B is saved at depth 3: only that segment is tabbable.
    expect(radios[2]).toHaveAttribute("aria-checked", "true");
    expect(radios[2]).toHaveAttribute("tabindex", "0");
    expect(radios[0]).toHaveAttribute("tabindex", "-1");

    // Clicking another segment moves the selection (draft enables save).
    await user.click(radios[0]);
    expect(radios[0]).toHaveAttribute("aria-checked", "true");
    expect(radios[2]).toHaveAttribute("aria-checked", "false");
    expect(screen.getByRole("button", { name: "保存配置" })).toBeEnabled();

    // Arrow keys move selection with focus following (roving tabindex).
    radios[0].focus();
