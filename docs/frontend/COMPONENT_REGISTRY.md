...
| SettingsPage | `features/settings/SettingsPage.tsx` | Local workspace settings: 外观主题 card (ADR-022 skin picker — see the theme store row below) first, then three wired cards — 桌面核心连接 (core.info status + capability versions) / AI Provider 密钥 (secrets.listProviders + setProviderKey, keychain-only) / 私有对话日志 (→ journal). The 外观主题 card also hosts the 界面语言 row (ADR-023 — see the language store row below): SegmentedControl 简体中文 / English, instant apply via the language store | 外观主题: instant-apply selection; 核心连接: loading/online/offline; 密钥: configured/unconfigured, save pending/error toast; 语言: instant-apply switch, persisted (`morpho.lang`) |
...
| i18n module + language store | `src/i18n/index.ts`, `stores/languageStore.ts` | Interface language, ADR-023 public contract: `language: LanguageId` (`"zh-CN" \| "en"` whitelist), `setLanguage` instant-applies through the i18n module's `setLanguage` (`i18n.changeLanguage` + `document.documentElement.lang` + `localStorage["morpho.lang"]`, so instance/DOM/storage cannot drift); `getInitialLanguage` is the resolution ladder (stored key → navigator zh-prefix → `en`), resources are bundled TS modules per namespace under `src/i18n/locales/{zh-CN,en}/` (zh-CN is the byte-identical reference locale and i18next fallback). Consumed by the SettingsPage 界面语言 row (SegmentedControl). **All extracted surfaces use the `t()` pattern** (ADR-023): `const { t } = useTranslation("<namespace>")`, literals become `t("key")` / `t("key", { param })`; namespaced data keys (e.g. `WORKSPACE_VIEWS.label = "shell:nav.plan"`) stay data-only in stores and are translated at render. To add a namespace: create `locales/zh-CN/<surface>.ts` + `locales/en/<surface>.ts`, register both in `src/i18n/resources.ts`, add the id to `NAMESPACES` in `src/i18n/index.ts`. **I2 coverage**: every view owns a namespace (`projects` `overview` `config` `plan` `tasks` `sources` `knowledge` `graph` `journal` `reports` `assistant`), shared card components own `cards`, and the remaining SettingsPage card copy extended `settings`; the documented display vocabularies (task/plan/confidence/node/dimension/source/task-kind/depth/assistant-action labels that used to render from `types/labels.ts`) resolve at render via `t("common:vocab.<group>.<value>", { defaultValue })` — `labels.ts` stays untouched because mock services compose fixture data from it | zh-CN default (test-pinned), persisted language, storage blocked (session-only language, no throw), unknown id ignored, en missing key → zh-CN fallback |
