...
AI assistant: floating launcher (fixed bottom-right gradient pill `brand-gradient-button`) opening a 360×530 popup panel above it; the docked-inspector pattern and its breakpoint rules (xl dock / <1024px overlay) are removed (ADR-013). Below lg the popup becomes a full-width bottom sheet (capped at `max-h-[70dvh]`) over a scrim that closes on tap, and the launcher stays mounted beneath it (narrow-window pass 2026-09-13). The sidebar drawer below lg is retained.
