...
集成任务按合并顺序串行执行，负责跨泳道冲突处理与完整质量检查；其后的本地计算闭环任务把研究引擎接到本机 Ollama 与可选真实搜索。

| ID | 结果 | 依赖 | 验收 |
|---|---|---|---|
| `INT-01` | 契约、Fixture 工具链与 Python 研究引擎分支集成入 main | `W2-01`–`W2-06`、`TEST-01`、`TEST-03`、`PY-01`–`PY-05`、`RES-01`–`RES-06` | 三组分支合并后各语言测试与契约/架构/边界检查全绿；跨泳道冲突显式处理并记录 |
| `PY-06` | Provider 路由 profile（default/local/offline） | `PY-05`、`W2-03` | profile 预设只改角色路由；显式 `MORPHO_ROLE_<ROLE>` 覆盖优先于 `MORPHO_PROFILE`；未知 profile 明确报错 |
| `PY-07` | SearXNG 搜索适配器（配置门控，默认 mock） | `PY-06`、`W2-03`、`RES-03` | 仅显式 `kind=search` 选择启用；端点安全规则与 SEARCH_FAILED 错误映射有测试；测试注入 transport 零真实网络 |
| `PY-08` | `run_research` CLI 入口与审批门禁 | `PY-06`、`RES-01`、`RES-02` | 无 `--approve` 只落 plan.json；`--approve` 后仅写白名单结果文件；退出码 0/1/2 语义有测试；离线演示零网络零凭据 |
...
| 分支集成与本地计算闭环 | 4 | 串行合入契约/工具链/引擎分支，补齐 profile 路由、SearXNG 与 CLI 闭环 |
| W4 集成与发布 | 4 | 串行端到端验证和人工发布审查 |

实际最多适合同时运行四条基础泳道，而不是为 49 个任务启动 49 个 AI。建议同时运行 4 到 8 个 AI：维护者或一个集成 AI 保留给契约、Migration、冲突处理和完整质量门禁。继续增加 AI 数量通常只会增加合并冲突，不会缩短关键路径。
